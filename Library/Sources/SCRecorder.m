...
- (void)recordAtTime:(CMTime)startTime {
  self.startTime = startTime;//CMClockGetTime(CMClockGetHostTimeClock());
  [self record];
}
  ...
  if (!recordSession.videoInitializationFailed && !_videoConfiguration.shouldIgnore) {
    ...
    if (!self.audioEnabledAndReady || recordSession.audioInitialized || recordSession.audioInitializationFailed) {
      ...
      if (_isRecording && recordSession.recordSegmentReady) {
        ...
        float bufferTime = (double)bufferTimestamp.value / (double)bufferTimestamp.timescale;
        float startTimestamp = (double) self.startTime.value / (double) self.startTime.timescale;
